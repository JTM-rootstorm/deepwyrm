...
fn production_and_six_memory_selector_artifacts_are_separated() {
    ...
    for selector in SELECTORS {
        ...
        validate_selector_stack_margin(selector, &stack_sizes(&llvm_readelf, &stack_artifact));
    }
...
fn build_stack_kernel(
    ...
    run_success(&mut command, &format!("{selector} stack-size build"));
    let artifact = target_dir.join("x86_64-unknown-none/debug/deepwyrm-kernel");
    assert!(
        artifact.is_file(),
...
fn validate_selector_stack_margin(selector: &str, sizes: &[StackSize]) {
